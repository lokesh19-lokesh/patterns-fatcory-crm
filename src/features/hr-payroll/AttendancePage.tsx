        ...
        <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-2 text-xs text-emerald-700 font-bold">
              <ShieldCheck className="w-4 h-4 text-emerald-600" /> Geofence Verified Zone: Plant HQ Mumbai
            </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {records.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-3.5 font-mono text-[#D8232A] font-bold">{formatDateTime(r.check_in_time)}</td>
                  <td className="p-3.5 font-mono text-slate-600 flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-amber-600" /> {r.check_in_lat}, {r.check_in_lng}
                  </td>
                  <td className="p-3.5 text-center font-bold text-amber-600">{r.overtime_hours} hrs</td>
